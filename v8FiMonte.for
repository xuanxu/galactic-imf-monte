...
96     end do
       close(15)
        lmt=lm1+lm2
c ----------------------------------------------- 
c  ---Generamos fi mediante un montecarlo---
      
       
c     montns es el numero total de masas solares
c     a generar mediante montecarlo
        montns=1000000.
         open(unit=15,file='masa_estrellas')
         read(15,*)montns
         close(15)
c     masas inicial y final para el montecarlo
        montmi=0.8
        montmf=100.
c       print*,montmi
        fimax=fiteorica(montmi) 
 
c       SEMILLA NUMERO ALEATORIO 

        open(33,file='semilla')
97      format (I6)
        read(33,97) isemran 
        close(33)
        seed=isemran*100000.
        ...
        if(isemran.lt.10)write(mod,101)isemran
        if(isemran.lt.100.and.isemran.ge.10)write(mod,102)isemran
        if(isemran.lt.1000.and.isemran.ge.100)write(mod,103)isemran
        if(isemran.ge.1000)write(mod,104)isemran
 ...
 104      format(I4)
        ...
        fimonte='fimonte_'//mod
        open(31,file=fimonte)
        CALL ZBQLINI(seed)
...
c         montx=Rand(0) 
         ...
         montm=montmi+montx*(montmf-montmi)

         montx=ZBQLU01()
         monty=fimax*montx          

         if(monty.lt.fiteorica(montm)) then 
 31      format(1x,4(E8.3,1x))
           ya=0
           do c=1,lmt
              if (ya.eq.1) exit               
              if (vm(c,1).le.montm.and.vm(c,2).gt.montm) then 
           ...
           h=h+montm
           vmontfi(montint)=vmontfi(montint)+montm
           vmontfi2(montint)=vmontfi2(montint)+1
           totstar=totstar+1
         end if      
        end do 
...
c   ahora Normalizo   
       do i=1,lmt                       
        ...
        vmontfi2(i)=(vmontfi2(i)/(vm(i,2)-vm(i,1)))/totstar  
         if(vm(i,1).ge.100)vmontfi2(i)=0.0
         if(vm(i,1).eq.vm(i,2))vmontfi2(i)=0.0
         write(31,31)vm(i,1),vm(i,2),vmontfi2(i),fiteorica(vm(i,1))
       end do  
c      print *,'Se han generado un total de', totstar, ' estrellas'
c      print *, 'hasta conseguir la masa total de ',h,'Msol'
       continue  
...
c  función fi generada por montecarlo 
       ...
       integer ok 
c      print *,'vmontfi(60) vale ', vmontfi(60)
       ok=0 
       do i=1,lmt
        if(ok.eq.1) exit 
        if (vm(i,1).ne.vm(i,2)) fi=0   
        if (m.le.vm(i,2).and.m.ge.vm(i,1).and.vm(i,1).ne.vm(i,2)) then    
          fi=vmontfi2(i)/2.38
c        if(m.eq.100.)print*,i,m,vm(i,2),vm(i,1),vmontfi2(i),fi
c factor 2.38 para normalizar con fiteorica
          ok=1 
        end if 
       if(m.le.vm(i,2).and.m.ge.vm(i,1).and.vm(i,1).eq.vm(i,2))fi=0.0
       end do  
c      if (m.ge.83.) then
c       print *,'i:',i,' m: ',m,' fith: ', fiteorica(m), ' fi: ',fi 
c      end if
       return 
       end 
c ------------------------------------------- 
